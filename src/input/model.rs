...
/// Example: login = ["user", "pass"] or mqtt session = ["connect", "publish"].
#[derive(Debug, Clone)]
pub struct SequenceSpec {
    pub name: String,
    /// Names of MessageSpec entries, in order.
...
pub struct ProtocolModel {
    ...
    /// Multi-step message sequences (optional).
...
impl ProtocolModel {
    ...
    pub fn ftp() -> Self {
        ...
        m.delimiter = Some(b'\n');
        // Lightweight text-oriented message templates for sequence synthesis
        m.messages = vec![
    ...
    /// Resolve a MessageSpec by name (case-insensitive).
